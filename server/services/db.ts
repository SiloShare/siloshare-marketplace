...
import {
  ...
  InsertMensagem,
  verificationCodes,
  documentosProdutores,
  InsertDocumentoProdutor,
  DocumentoProdutor,
} from "../drizzle/schema";
...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    ...
    const textFields = ["name", "email", "telefone", "cpfCnpj", "loginMethod"] as const;
...
export async function getSilos(filters?: {
  ...
  // ✅ SEMPRE filtrar apenas silos aprovados na busca pública
  conditions.push(eq(silos.status, 'aprovado'));
...
export async function getSilosParaAprovacao(): Promise<Silo[]> {
  ...
  return await db
    .select()
    .from(silos)
    .where(eq(silos.status, 'pendente'))  // ✅ Filtrar apenas silos pendentes
    .orderBy(desc(silos.createdAt));
}
...
export async function aprovarSilo(siloId: number, adminId: string): Promise<Silo | undefined> {
  ...
  // Validar se já foi aprovado
  if (siloAtual.status === "aprovado") {
    throw new Error("Silo já foi aprovado anteriormente");
  }

  // Buscar informações do fornecedor
  const fornecedor = await db.select().from(users).where(eq(users.id, siloAtual.userId)).limit(1);
  
  // Atualizar status (usando campo correto do schema)
  await db
    .update(silos)
    .set({
      status: "aprovado",  // ✅ Campo correto do schema
      updatedAt: new Date(),
    })
...
export async function reprovarSilo(siloId: number, motivo: string, adminId: string): Promise<Silo | undefined> {
  ...
  // Validar se já foi reprovado
  if (siloAtual.status === "reprovado") {
    throw new Error("Silo já foi reprovado anteriormente");
  }

  // Buscar informações do fornecedor
  const fornecedor = await db.select().from(users).where(eq(users.id, siloAtual.userId)).limit(1);

  // Atualizar status (usando campos corretos do schema)
  await db
    .update(silos)
    .set({
      status: "reprovado",      // ✅ Campo correto do schema
      motivoReprovacao: motivo,  // ✅ Campo correto do schema
      updatedAt: new Date(),
    })
    .where(eq(silos.id, siloId));
